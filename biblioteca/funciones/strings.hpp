...
string substring(string s, int d, int h)
{
    ...
    assert(h <= length(s) &&
           "El indice hasta debe ser menor o igual a la longitud de la cadena");
    string x = "";
    for(int i = d; i < h; i++)
    {
        x += s[i];
...
string substring(string s, int d)  // ok
{
    assert(d >= 0 && "El indice desde debe ser mayor o igual a 0");
    assert(d <= length(s) &&
           "El indice desde debe ser menor o igual a la longitud de la cadena");
...
int indexOf(string s, char c, int offSet)  // ok
{
    ...
    assert(offSet <= length(s) &&
           "El offset debe ser menor o igual a la longitud de la cadena");
...
int indexOf(string s, string toSearch)  // ok
{
    ...
    assert(length(toSearch) <= length(s) &&
           "La subcadena a buscar no puede ser mas larga que la cadena");
    int ns = length(s);
...
int indexOfN(string s, char c, int n)
{
    ...
    for(int i = 0; i < n; i++)
    {
        ...
        assert(x >= 0 &&
               "La cadena no contiene la cantidad de ocurrencias solicitadas");
        offset += x + 1;
    }
    return offset - 1;
}
...
{
    assert(isLetter(c) ||
           isDigit(c) && "El caracter debe ser un digito entre '0' y '9'");
...
int stringToInt(string s, int b)  // ok
{
    ...
    for(int i = 0; i < n; i++)
    {
        ...
        assert(d < b &&
               "Todos los digitos de la cadena deben ser menores que la base");
        sum += d * poww(b, i);
    }
...
bool startsWith(string s, string x)
{
    assert(length(x) <= length(s) &&
           "El prefijo no puede ser mas largo que la cadena");
...
string removeAt(string s, int pos)
{
    assert(pos >= 0 && "La posicion debe ser mayor o igual a 0");
    assert(pos < length(s) &&
           "La posicion debe ser menor que la longitud de la cadena");
...
{
    assert(c != NULL &&
           "El puntero al arreglo de caracteres no puede ser nulo");
    return string(c);
}
